...
- タスクの編集(タスク名・期限日・優先度・カテゴリ。タスク名クリックまたは「編集」ボタン)
- カテゴリ(研究 / 就活 / 授業 / その他)の設定とバッジ表示
- タスク名の検索、優先度・カテゴリでの絞り込み(リスト・カレンダー両方に反映)
- リスト表示の並び替え(追加した順 / 期限日が近い順 / 優先度が高い順)
...
- **並び順の自動整理**: 未完了タスクは「優先度が高い順 → 期限が近い順」に並びます。
...
- 空タスクは追加できないようにし、完了済みセクションは折りたたみ・一括削除が可能です。
